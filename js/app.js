(() => {
    ...
    const tripCompleted = document.querySelector("#ongoing");
    modules_flsModules.tippy = tippy_esm("[data-tippy-content]", {
        content: tripCompleted,
        interactive: true,
        allowHTML: true,
        arrow: false
    });
    ...
    function initSliders() {
        if (document.querySelector(".main-block__swiper")) new swiper_core_Swiper(".main-block__swiper", {
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        document.addEventListener("click", (e => {
            ...
            if (isMobile.any() && screenWidth > 767.98) {
                const buttons = document.querySelectorAll("[data-hover]");
                if (buttons.length) if (target && target.hasAttribute("data-hover")) target.parentNode.classList.toggle("_hover"); else if (!target.closest("._hover") && document.querySelectorAll("._hover").length > 0) buttons.forEach((button => {
                    button.parentNode.classList.remove("_hover");
                }));
            }
        }));
        infiteAutoSlider();
